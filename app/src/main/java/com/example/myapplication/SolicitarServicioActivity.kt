...
class SolicitarServicioActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val txtDescripcion = findViewById<EditText>(R.id.txtDescripcionProblema)
        val txtMarca = findViewById<EditText>(R.id.txtMarcaModelo)
        val txtEspecificaciones = findViewById<EditText>(R.id.txtEspecificaciones)

        val btnEnviar = findViewById<Button>(R.id.btnEnviarSolicitud)
        val btnMisSolicitudes = findViewById<Button>(R.id.btnMisSolicitudes)
        val btnRegresar = findViewById<Button>(R.id.btnRegresarSolicitud)
        ...
        btnEnviar.setOnClickListener {
            ...
            val especificaciones = txtEspecificaciones.text.toString().trim()

            if (desc.isEmpty() || marca.isEmpty()) {
                Toast.makeText(this, "Por favor, ingresa el problema y la marca/modelo.", Toast.LENGTH_SHORT).show()
            ...
            val nuevoServicio = Servicio(
                ...
                precio = 0.0, // El técnico define el precio después
                movilNombre = marca,
                movilEspecificacion = especificaciones,
                fecha = fechaActual,
                etapa = 0 // 0 = Recibido/En revisión
            )
            ...
            api.agregarServicio(token, nuevoServicio).enqueue(object : Callback<Void> {
                override fun onResponse(call: Call<Void>, response: Response<Void>) {
                    if (response.isSuccessful) {
                        ...
                        txtMarca.text.clear()
                        txtEspecificaciones.text.clear()
                    } else {
